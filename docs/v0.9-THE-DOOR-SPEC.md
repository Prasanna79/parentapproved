...
- Create a GCP service account with Firebase Test Lab permissions. No `google-services.json` needed — the app has zero Firebase dependencies. We only use `gcloud` CLI to upload APKs and run tests
- In `release.yml`, after `assembleRelease` and `assembleDebugAndroidTest`:
  ...
  - `--results-dir` and `--results-bucket` for test result artifacts
...
- **Cost:** Blaze pay-as-you-go is ~$1/hour of virtual device time. A 5-minute test run costs ~$0.10. Even running weekly, annual cost is under $5. Set a budget alert at $5/month as a safety net
...
- GitHub Actions secrets: `FIREBASE_SERVICE_ACCOUNT` (service account JSON, base64-encoded)
